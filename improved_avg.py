...
def aggregate(global_model, models, data_size, client_losses):
    """
    聚合客户端的模型参数，根据数据量和损失动态分配权重。

    参数：
        global_model: 全局模型
        models: 客户端本地模型列表
        data_size: 每个客户端本地数据量
        client_losses: 每个客户端的损失列表
    ...
    with torch.no_grad():
        ...
        global_model_dict = dict(global_model.state_dict())
        aggregated_dict = dict(global_model.state_dict())
        parties_dict = {i: dict(models[i].state_dict()) for i in range(len(models))}

        # 归一化客户端的数据量
        data_size = np.array(data_size) / sum(data_size)

        # 归一化客户端的损失（使用倒数）
        loss_weights = 1 / (np.array(client_losses) + 1e-8)  # 避免除以零
        loss_weights /= sum(loss_weights)

        # 计算综合权重
        combined_weights = data_size * loss_weights  # 每个客户端综合权重
        combined_weights /= sum(combined_weights)   # 再次归一化，确保权重之和为 1

        # 按权重聚合每一层参数
        for name, param in global_model_dict.items():
            aggregated_dict[name].data.copy_(
                sum([combined_weights[i] * parties_dict[i][name].data for i in range(len(models))])
            )
